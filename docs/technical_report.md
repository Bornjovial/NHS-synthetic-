...
Data is only as useful as the signal it carries, the clear patterns that drive predictions. Real-world data is often messy, biased, or restricted, obscuring that signal. Synthetic data is designed to preserve the signal while controlling the noise. Done right, it unlocks new possibilities for AI, research, and privacy-conscious analysis; done wrong, it distorts insights and misleads models.

Projects surrounding clinical documentation involve some of the most sensitive data and must prioritise privacy. In many cases, starting with real data and applying privacy enhancing techniques does not pass approvals for research and development.

Synthetic data is a promising alternative to real data that addresses privacy concerns which are commonly encountered in healthcare research ([Murtaza et al]([https://www.sciencedirect.com/science/article/abs/pii/S1574013723000138), [Pezoulas et al](https://www.sciencedirect.com/science/article/pii/S2001037024002393)). Recent developments in Large Language Models (LLMs) make the creation of unstructured synthetic text data significantly more accessible.

This document describes work done to generate synthetic clinical notes. By developing and evaluating new AI tools with synthetic data, you can assess a tools performance while avoiding privacy concerns associated with real patient records.

### 1.1 Requirements

**Realism**

The synthetic notes need to exhibit medical realism and maintain compatibility with existing data systems. They should mirror the structure and content of real patient notes, which will allow new AI tools to generalise effectively to real-world clinical documentation. Importantly, the synthetic notes should show internal consistency, meaning that the notes written about a patient are describing the same underlying series of events that make up their journey through hospital. This is a key requirement that drove many of the design decisions underlying our note generation pipeline.

**Variation**

Additionally, the synthetic notes must capture the broad range of variations found in real clinical documentation and should be steerable to allow for specific testing. AI tools should be able to handle differences in writing style, terminology, and formatting across specialties and clinical settings. The synthetic dataset should reflect variations in the completeness of notes, levels of detail, and clinician-specific documentation habits to ensure the system performs reliably across diverse scenarios.

The exact requirements of this project changed during development. For this reason, we needed our synthetic note generation pipeline to be flexible enough to adapt to a variety of possible testing requirements. This drove a lot of the configuration options that we developed.
...
1. The patient demographics and their reason for hospitalisation can be represented by structured data;
2. The patient’s journey through hospital is semi-structured;
3. The final outputs, i.e. the clinical notes themselves, constitute unstructured data.
  
Sections 2.1 – 2.3 discuss existing work that relates to each of these synthetic data types.

### 2.1 High Privacy Structured Data Generation
...
The simplest way of doing this is to “erode” raw data (through adding noise, suppressing low counts, reallocation and aggregation) until a privacy threshold (usually defined by k-anonymity, l-diversity, or t-closeness ([Olatunji et al](https://arxiv.org/pdf/2104.06523)) is achieved. The NHS [Digital Artificial Data pilot](https://digital.nhs.uk/services/artificial-data) is one such example of this.  

Alternatively, we can use tools such as [Synthea](https://synthetichealth.github.io/synthea/). This is an open-source patient population simulator which allows for the generation of machine readable, realistic and synthetic patient data. This can then generate lifespans of realistic but fictional synthetic patient electronic health records. Alternative simulation engines including agent-based modelling (Agent Hospital ([Li et al](https://arxiv.org/abs/2405.02957))) and digital twin setups which have been developed but often turn out to be too burdensome to maintain for general usage.
...
1. The inherent performance of the LLM on medical tasks (i.e., its medical knowledge and reasoning capabilities).
2. The prompting techniques used to guide the model's responses.

A common error experienced by LLMs is hallucinations – generating plausible but factually incorrect content ([cosio et al](https://arxiv.org/pdf/2508.01781)). For many medical tasks this is problematic, particularly when using LLMs to generate discharge summaries. Various studies have been done on LLM performance on medical tasks. For example, [Kim et al](https://www.medrxiv.org/content/10.1101/2025.02.28.25323115v1.full.pdf) investigated the hallucination rates of several state-of-the-art LLMs and found significant disparities in performance across models. Here, OpenAI’s GPT-4o exhibited the highest hallucination rate. Some models are specifically trained for medical tasks. Google has released Med-Gemini ([Saab et al](https://arxiv.org/pdf/2404.18416)), and other studies have developed domain-specific medical LLMs, such as GatorTronGPT and Me-LLaMA ([Peng et al](https://www.nature.com/articles/s41746-023-00958-w#Sec8)).
...
There are further steps we can take to improve the quality of our clinical notes. One common method is prompt engineering: the iterative refinement of prompts to elicit better outputs. Chain-of-Thought prompting ([Wei et al](https://proceedings.neurips.cc/paper_files/paper/2022/file/9d5609613524ecf4f15af0f7b31abca4-Paper-Conference.pdf)), which encourages models to reason step-by-step before answering, has been shown to also improve performance across tasks.

LLM validators can also be incorporated into LLM pipelines. These are LLMs prompted to judge and improve outputs based on specific criteria. Using validators, [Gosmar et al](https://arxiv.org/pdf/2501.13946) demonstrated a successful three-agent system that detected and removed hallucinations from LLM-generated outputs.

LLM performance can also be enhanced through access to additional knowledge. We define ‘additional knowledge’ broadly, ranging from in-context learning to the use of knowledge graphs. In-context learning enables LLMs to generate responses that mirror examples provided within the prompt ([He et al](https://www.sciencedirect.com/science/article/pii/S1566253525000363)).  Retrieval-Augmented Generation (RAG) allows LLMs to search databases and retrieve relevant information, and has been shown to improve performance in medical tasks ([Xiong et al](https://arxiv.org/abs/2402.13178)). Some studies have further improved retrieval methods by incorporating structured resources such as knowledge graphs ([Jiang et al](https://arxiv.org/abs/2410.04585)). Notably, many of these techniques can be used in combination for greater effect.

Alongside synthetic data generation, LLMs can also be used to evaluate the outputs of other LLMs - known as LLM-as-a-Judge ([Gu et al](https://arxiv.org/pdf/2411.15594)). We have already discussed one type of LLM Judge: a validator, but LLM Judges can also grade, evaluate, critique, and rank responses . However, an LLM-as-a-Judge is not perfect. LLM Judges can show bias, including favouring the first response when comparing two outputs, or preferring longer responses ([Ye et al](https://arxiv.org/pdf/2410.02736)). LLM judges can themselves be evaluated. One approach is to assess their alignment with human judgments using metrics such as Cohen’s kappa ([Thakur et al](https://arxiv.org/pdf/2406.12624v1)).

## 3. Methodology

```mermaid
flowchart LR
    A[Generate Patients] --> B[Generate Reasons for Admission]
    B --> C[Generate Patient Journeys]
    C --> D[Generate Clinical Notes]
    D --> E[Augment Clinical Notes]

    classDef step fill:#a8d0e6,stroke:#333,stroke-width:1px,color:#000;
    class A,B,C,D,E step;
```

**Diagram 1**: A simple overview of our pipeline.
...
Diagram 1 shows an overview of the methodology we used to create the synthetic notes. There are five high-level stages to this.

Alongside this pipeline is an evaluation suite to aide iterative improvement of the pipeline. This will be discussed in Section 3.6. 

1. Generate Patients
2. Generate Patient Admissions
3. Generate Patient Journeys
4. Generate Clinical Notes
5. Augment Clinical Notes

### 3.1 Generating Patients

```mermaid
flowchart LR
    A[Synthea Data] --> B["Sample patient data<br/>e.g. name, GP, allergies"]
    B --> C["🤖 Generate patient data<br/>e.g. NOK, age"]
    C --> D[Save Outputs]

    classDef synthea fill:#cfcfcf,stroke:#333,color:#000;
    classDef sample fill:#bfbfbf,stroke:#333,color:#000;
    classDef generate fill:#f3c6a6,stroke:#333,color:#000;
    classDef save fill:#2c7da0,stroke:#333,color:#fff;
...
**Diagram 2**: Generating a population of patients. Synthea data is used alongside an LLM to create a population of varied but realistic patients.

In order to generate basic patient information such as name, age and gender, we use Synthea. As Synthea was originally developed in a US context, for this project we use Synthea UK , a modified version of Synthea developed by NHSE. [Synthea UK](https://nhsengland.github.io/swpc_synthea/) This version is designed so that the demographics and addresses of generated patients align with the UK population.

It should be noted that Synthea is a relatively complex bit of software and that we only use the first step of its data generation pipeline, initial patient generation, in this work. Synthea UK is focused on primary care - to use it later in our pipeline we would require secondary care journeys and development of a secondary care simulation engine was not possible in the time available. Patient names are generated using a random combination of the most common given names and surnames in the UK.
...
### 3.2 Generating Admissions

```mermaid
flowchart LR
    A[Reason for Admission Data]
    B[Patients]
    C["🤖 Generate <b>elective</b><br/>and <b>emergency</b><br/>admissions "]
    D["🤖 Generate<br/><b>length of stay</b> "]
    E[Save Outputs]
    ...
    %% Styles
    classDef grey fill:#cfcfcf,stroke:#2f4f5f,stroke-width:2px,color:#000;
    classDef blue fill:#2b6f8a,stroke:#2f4f5f,stroke-width:2px,color:#fff;
    classDef peach fill:#efc4a8,stroke:#2f4f5f,stroke-width:2px,color:#000;
...
### 3.3 Generating PAtient Journeys using an LLM

```mermaid
flowchart LR
    PA([Patients Admissions]) --> GS["🤖 Generate <b>simple</b>  patient journey"]
    GS --> CJ["🤖 Check journey complete"]
    CJ --> EJ["🤖 Expand Journey"]
    EJ --> CJ
    PA --> VJ["🤖 Validate patient journey"]
    CJ --> VJ
    VJ --> AD["🤖 Add  <b>details</b>"]
    AD --> FE["Filter events"]
    FE --> SO[Save Outputs]

    classDef admissions fill:#2c7da0,stroke:#333,color:#fff;
    classDef robot fill:#f3c6a6,stroke:#333,color:#000;
    classDef neutral fill:#cfcfcf,stroke:#333,color:#000;
    classDef save fill:#2c7da0,stroke:#333,color:#fff;

    class PA admissions;
    class GS,CJ,EJ,VJ,AD robot;
    class FE neutral;
    class SO save;
```

**Diagram 4**: Generating patient journeys. Patient journeys are generated using a series of LLM calls.

This stage (Diagram 4) of the pipeline uses multiple LLM calls to generate realistic patient journeys. A patient journey is a set of events starting form the point of admission up until the moment just before discharge. This method was chosen due to its adaptability and no reliance on real data.
...
### 3.4 Generating Clinical Notes using LLMs

```mermaid
flowchart LR
    A[Patients Admissions Journeys]
    B[Adapt Prompts]
    C["🤖 Generate Clinical Notes"]
    D["🤖 Validate Clinical Notes"]
    E[Save Outputs]
...
**Diagram 5**: Generating clinical notes. Our clinical note generation stage uses very versatile prompts to tailor the prompt to each note type, patient and event. Once clinical notes are generated, a validator checks the faithfulness of the note to the event description.

This stage (Diagram 5) of the pipeline uses the detailed description of each event to generate a single clinical note per event.

#### 3.4.1 Generating Clinical Notes from Patient Journeys
...
**Diagram 6**: We augment our clinical notes by optionally adding typos, abbreviations, and staff sign-offs.

As seen in Diagram 6, this stage of the pipeline adds abbreviations, adds typos and adds sign-offs to notes.

#### 3.5.1 Adding Abbreviations
...
However, we found that some abbreviations have meanings too complex to simply use a find a replace for. For that reason, we designed an LLM prompt to add abbreviations to a string. As it is an LLM, it can add abbreviations that flow in the sentence.

#### 3.5.2 Typo Generation

Following clinical feedback, we used the python [typo](https://pypi.org/project/typo/) package to add realistic typos to our clinical notes. This package aims to simulate realistic typos by using techniques such as:
...
### 3.6 Evaluation and Iterative Improvement

```mermaid
flowchart LR
    A[Evaluation Data]
    B["🤖 LLM Judges: Fluency, Groundedness, Relevance"]
    C[Other metrics: Readability, Temporal Coherence]
    D[Evaluation Report]
...
**Diagram 7**: Generation of an evaluation report using traditional metrics and LLM Judges.

We built an evaluation pipeline to evaluate the quality of our clinical notes during development, allowing us to iteratively improve our prompts and pipeline for better clinical notes.
...
We have selected the Flesch Reading Ease Score and Dale Chall Readability Score from textstat for our Evaluations.
The Flesche Reading Ease Score considers the total words, total sentences and total syllables to measure the readability. Number range from 0 to 100, with 0 being difficult to read and 100 being very easy to read.

The Dale Chall Readability Score uses a lookup table of the most commonly used English words. Scores of 4.9 or lower are easier to read (approximately a 4th-grade student), and 9+ is hard to read. As we have used textstat, it is very easy to substitute in more readability formula.
To summarise the score for the entire dataset, we take the mean average of both readability formula.

#### 3.6.2 LLM-as-a-Judge
...
#### 3.6.3 Temporal Coherence

We want to ensure each clinical note is dated such that events occur in order. This is a simple test that looks at the time stamps of each event for each patient, and checks whether the next event occurs at the same time or after.
Each patient returns a True if all events are in order, or a False if any events are not. To calculate a score for the entire dataset, the fraction of patients where all events are in time order is returned.
...
| Issue Identified | Improvement Implemented |
|------------------|-------------------------|
...
| Notes were too verbose | Prompt engineering |
| Note structure and subheadings were unrealistic | Modifying the expected output template which is passed into the LLM prompt |
| It was unclear what the role of the clinicians writing the notes was | Be clearer about the roles of synthetic clinicians and include sign-offs in the templates for each note type |
| Some abbreviations were unrealistic | Change the approach to abbreviations to use an LLM |
...
#### 3.7.2 Novel and Rare Disease Sampling
...
- Including a configurable parameter to control whether information about previous events is included in the note prompt and note validation prompt
- Changing the ward round templates throughout the patient stay so that later ward round notes are more concise
...
This document describes the methodology behind our synthetic clinical note generation. The process follows the latest research in LLM usage and evaluation and has been iteratively refines following clinical feedback of the clinical notes.
