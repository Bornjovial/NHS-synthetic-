from prompts import patient_and_admission_prompts, patient_journey_prompts, clinical_note_prompts
from doc_templates import document_templates, template_sections_to_combine
from dataset_utils import style_instructions, allergy_prevalence, generate_staff_gmc_and_pin, add_sign_off_to_note, prepare_note_data, prepare_patient_data, prepare_admission_data, prepare_encounter_data, prepare_evaluation_data, write_dataset, format_note, get_journey_evaluation_details, generate_staff_gmc_and_pin
from processing import call_llm, call_llm_async, read_write_data, clean_outputs, remove_failures, clean_int, combine_patients_and_admissions, clean_patient_details, combine_template_sections, add_abbreviations_to_dict, add_typos_to_dict, normalise_array_struct_column, create_admission_window, random_24_hour_time, build_output_info
...
class generate_patients():
    ...
    def __init__(
        ...
        # Model
        self.model = model if model is not None else "GPT_4o"
    ...
    async def generate_patients(self, last_name_only = False):
        ...
        allergies = [self.generate_patient_allergies(allergy_prevalence) for i in range(N)]
        ages = [random.randint(
...
class generate_journeys():
    ...
    def generate_staff_personas(self, staff_names, abbreviates_content_prob, abbreviates_headers_prob):
        ...
        for staff in staff_names:
            ...
            persona = random.choice(list(style_instructions.keys()))
            staff_personas[staff]["style"] = style_instructions[persona]
